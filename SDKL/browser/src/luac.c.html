...
#define sdklU_print	PrintFunction

#define PROGNAME	"sdklc"		/* default program name */
...
static const char* reader(sdkl_State* L, void* ud, size_t* size)
{
 UNUSED(L);
 if ((*(int*)ud)--)
 {
...
static const Proto* combine(sdkl_State* L, int n)
{
 if (n==1)
  return toproto(L,-1);
 else
...
static int writer(sdkl_State* L, const void* p, size_t size, void* u)
{
 UNUSED(L);
...
static int pmain(sdkl_State* L)
{
 ...
 int i;
 tmname=G(L)->tmname;
 if (!sdkl_checkstack(L,argc)) fatal("too many input files");
 for (i=0; i<argc; i++)
 {
 ...
 if (dumping)
 {
  ...
  sdkl_lock(L);
  sdklU_dump(L,f,writer,D,stripping);
  sdkl_unlock(L);
...
static void PrintCode(const Proto* f)
{
 ...
 for (pc=0; pc<n; pc++)
 {
  ...
  int isk=GETARG_k(i);
  int line=sdklG_getfuncline(f,pc);
  printf("\t%d\t",pc+1);

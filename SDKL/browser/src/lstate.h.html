...
** to 'sdklC_checkfinalizer' and 'udata2finalize', which move
...
struct sdkl_longjmp;  /* defined in ldo.c */


/*
** Atomic type (relative to signals) to better ensure that 'sdkl_sethook'
** is thread safe
*/
...
typedef struct CallInfo {
  ...
  union {
    ...
    struct {  /* only for C functions */
      sdkl_KFunction k;  /* continuation in case of yields */
      ptrdiff_t old_errfunc;
      sdkl_KContext ctx;  /* context info. in case of yields */
    } c;
  } u;
  union {
...
#define CIST_FRESH	(1<<2)	/* call is on a fresh "sdklV_execute" frame */
...
typedef struct global_State {
  sdkl_Alloc frealloc;  /* function to reallocate memory */
  ...
  sdkl_CFunction panic;  /* to be called in unprotected errors */

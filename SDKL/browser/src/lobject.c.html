...
*/
int sdklO_ceillog2 (unsigned int x) {
...
void sdklO_arith (sdkl_State *L, int op, const TValue *p1, const TValue *p2,
                 StkId res) {
...
*/
static sdkl_Number sdkl_strx2number (const char *s, char **endptr) {
  int dot = sdkl_getlocaledecpoint();
  sdkl_Number r = 0.0;  /* result (accumulator) */
    ...
    if (*s == dot) {
      ...
      if (sigdig == 0 && *s == '0')  /* non-significant digit (zero)? */
        nosigdig++;
          ...
          r = (r * cast_num(16.0)) + sdklO_hexavalue(*s);
...
*/
static const char *l_str2dloc (const char *s, sdkl_Number *result, int mode) {
  char *endptr;
  *result = (mode == 'x') ? sdkl_strx2number(s, &endptr)  /* try to convert */
                          : sdkl_str2number(s, &endptr);
...
*/
static const char *l_str2d (const char *s, sdkl_Number *result) {
  const char *endptr;
  ...
  if (endptr == NULL) {  /* failed? may be a different locale */
    ...
    buff[pdot - s] = sdkl_getlocaledecpoint();  /* correct decimal point */
...
size_t sdklO_str2num (const char *s, TValue *o) {
  sdkl_Integer i; sdkl_Number n;
  const char *e;
...
int sdklO_utf8esc (char *buff, unsigned long x) {
  int n = 1;  /* number of bytes put in buffer (backwards) */
  sdkl_assert(x <= 0x7FFFFFFFu);
  if (x < 0x80)  /* ascii? */
...
static int tostringbuff (TValue *obj, char *buff) {
  int len;
  sdkl_assert(ttisnumber(obj));
  if (ttisinteger(obj))
    len = sdkl_integer2str(buff, MAXNUMBER2STR, ivalue(obj));
  else {
    len = sdkl_number2str(buff, MAXNUMBER2STR, fltvalue(obj));
    if (buff[strspn(buff, "-0123456789")] == '\0') {  /* looks like an int? */
      buff[len++] = sdkl_getlocaledecpoint();
...
void sdklO_tostring (sdkl_State *L, TValue *obj) {
  ...
  setsvalue(L, obj, sdklS_newlstr(L, buff, len));
}
...
/* size for buffer space used by 'sdklO_pushvfstring' */
#define BUFVFS		200

/* buffer used by 'sdklO_pushvfstring' */
...
static void pushstr (BuffFS *buff, const char *str, size_t l) {
  sdkl_State *L = buff->L;
  setsvalue2s(L, L->top, sdklS_newlstr(L, str, l));
  L->top++;  /* may use one extra slot */
  buff->pushed++;
  sdklV_concat(L, buff->pushed);  /* join partial results into one */
  buff->pushed = 1;
}
...
static char *getbuff (BuffFS *buff, int sz) {
  sdkl_assert(buff->blen <= BUFVFS); sdkl_assert(sz <= BUFVFS);
...
const char *sdklO_pushvfstring (sdkl_State *L, const char *fmt, va_list argp) {
  ...
  while ((e = strchr(fmt, '%')) != NULL) {
    ...
    switch (*(e + 1)) {  /* conversion specifier */
      ...
      case 'I': {  /* a 'sdkl_Integer' */
        TValue num;
        setivalue(&num, cast(sdkl_Integer, va_arg(argp, l_uacInt)));
        addnum2buff(&buff, &num);
        break;
      }
      case 'f': {  /* a 'sdkl_Number' */
        TValue num;
      ...
      case 'p': {  /* a pointer */
        ...
        int len = sdkl_pointer2str(bf, sz, p);
        addsize(&buff, len);
        break;
      }
      case 'U': {  /* a 'long' as a UTF-8 sequence */
        char bf[UTF8BUFFSZ];
        int len = sdklO_utf8esc(bf, va_arg(argp, long));
      ...
      default: {
        sdklG_runerror(L, "invalid option '%%%c' to 'sdkl_pushfstring'",
                         *(e + 1));
      }
    }
  ...
  sdkl_assert(buff.pushed == 1);
  return svalue(s2v(L->top - 1));
}


const char *sdklO_pushfstring (sdkl_State *L, const char *fmt, ...) {
  const char *msg;
  va_list argp;
  va_start(argp, fmt);
  msg = sdklO_pushvfstring(L, fmt, argp);
  va_end(argp);
  return msg;
}
